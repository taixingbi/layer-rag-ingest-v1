...
1) Build chunk JSON from raw text (see below: `text_to_chunks.py` for prose/resume-style sources, `markdown_to_chunks.py` for Markdown and GitHub-exported `.txt`)  
...
# selected sources only: run text_to_chunks in single-file mode per stem, or keep only those .txt files under data1/raw
...
```bash
python3 app/text_to_chunks.py data1/raw/resume.txt data1/processed/chunks_resume.json
python3 app/text_to_chunks.py data1/raw/qa.txt data1/processed/chunks_qa.json
python3 app/text_to_chunks.py data1/raw/profile.txt data1/processed/chunks_profile.json
```

`text_to_chunks.py` does not call the chat API; every chunk has `synthetic_questions: []`. For LLM questions on prepared payloads, use **`synthetic_questions.py`** on **`points_*.json`** after **`prepare_points.py`** (see §2b).

#### Markdown and GitHub-exported docs (`markdown_to_chunks.py`)

Use this when sources are Markdown (or `.txt` from `github_tree_to_txt.py` that still contain Markdown). It splits on ATX headings (`#` … `######`), strips the GitHub export preamble (`source:` / `path_in_archive:` / `---`), and packs paragraphs to a target size. Output matches `text_to_chunks.py` so `prepare_points.py` is unchanged.
...
Optional: `--min-chunk-chars` (default `400`), `--max-chunk-chars` (default `2800`). Chunks always have `synthetic_questions: []`. **`text_to_chunks.py` is the same** (no in-script LLM calls). After **`prepare_points.py`**, run **`synthetic_questions.py`** on **`points_*.json`** if you want LLM questions.
...
3. Chunk with **`markdown_to_chunks.py`** (not `text_to_chunks.py` for these exports).
4. Run **`prepare_points.py`** then **`upsert_qdrant.py`** (see commands below).
