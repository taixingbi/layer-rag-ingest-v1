...
if [[ "${RUN_SYNTHETIC_QUESTIONS:-1}" == "0" ]]; then
  echo "==> data1 (${DATA_ENV}): skipping synthetic questions (RUN_SYNTHETIC_QUESTIONS=0)"
else
  echo "==> data1 (${DATA_ENV}): synthetic questions (requires chat / inference env)"
  "$PYTHON" app/synthetic_questions.py --data-dir "$DATASET_ROOT/processed" --questions-per-chunk 3
fi

echo "==> data1 (${DATA_ENV}): embed + upsert to Qdrant"
"$PYTHON" app/upsert_qdrant.py --data-dir "$DATASET_ROOT/processed" --pattern "points_*.json"

if [[ "${RUN_SMOKE_VALIDATE:-1}" == "0" ]]; then
  echo "==> data1 (${DATA_ENV}): skipping smoke validation (RUN_SMOKE_VALIDATE=0)"
else
  if [[ "${RUN_SMOKE_JUDGE:-1}" == "0" ]]; then
    echo "==> data1 (${DATA_ENV}): post-upsert smoke validation (judge disabled via RUN_SMOKE_JUDGE=0)"
    "$PYTHON" app/smoke_validate.py --data-dir "$DATASET_ROOT/processed" --pattern "points_*.json"
  else
    echo "==> data1 (${DATA_ENV}): post-upsert smoke validation (LLM judge enabled)"
...
if [[ "${RUN_RECONCILE:-0}" == "1" ]]; then
  echo "==> data1 (${DATA_ENV}): lifecycle reconcile"
...
echo "==> data1 (${DATA_ENV}) pipeline finished"
